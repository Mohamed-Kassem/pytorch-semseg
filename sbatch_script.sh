...
then
    ...
    BATCH_SIZE=1
    export CUDA_VISIBLE_DEVICES=0
    python train.py --arch fcn8s --dataset pascal --n_epoch 150 --img_rows 256 --img_cols 256 --batch_size ${BATCH_SIZE} --cuda_index 0 --l_rate 0 > "${SLURM_JOB_ID}_batch_size_${BATCH_SIZE}_fcn8s.out" 2>&1 &
    ...
    export CUDA_VISIBLE_DEVICES=1
    python train.py --arch fcn8s --dataset pascal --n_epoch 150 --img_rows 256 --img_cols 256 --batch_size ${BATCH_SIZE} --cuda_index 0 --kassem --l_rate 0 > "${SLURM_JOB_ID}_kassem_batch_size_${BATCH_SIZE}_fcn8s.out" 2>&1 &
