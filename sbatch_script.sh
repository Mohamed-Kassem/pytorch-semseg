...
then
    ...
    export CUDA_VISIBLE_DEVICES=1
    python train.py --arch segnet --dataset pascal --n_epoch 150 --img_rows 256 --img_cols 256 --batch_size ${BATCH_SIZE} --cuda_index 0 --l_rate 1e-4 > "${SLURM_JOB_ID}_batch_size_${BATCH_SIZE}_segnet.out" &
else
